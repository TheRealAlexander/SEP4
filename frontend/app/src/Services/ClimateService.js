    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(
          `${API_BASE_URL}/Broker/GetSensorData/1`
        );
        ...
        setThermData({
          temperature: "CatchBlock activated, network might be down!",
        });
        setIsLoading(false);
      }
    };
...
export async function setPreferredTemperature(prefTemp) {
  try {
    const response = await axios.post(
      `${API_BASE_URL}/setPreferredTemperature`,
      ...
      try {
        ...
        setHumidityData((prevData) => {
          ...
          return {
            ...
            readings:
              newReadings.length > 10 ? newReadings.slice(1) : newReadings,
            average: newAverage,
          };
        });
...
export async function setPreferredHumidity(prefHumi) {
  try {
    const response = await axios.post(`${API_BASE_URL}/setPreferredHumidity`, {
      humidity: prefHumi,
    });
    return response.data;
  } catch (error) {
