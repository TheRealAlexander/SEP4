...
function HomePage() {
  ...
  const [tempControlStates, setTempControlStates] = useState({
    ...controlStates,
  });
  ...
  const updateLightIntensity = (hal, laneId, newValue) => {
    setTempControlStates((prev) => ({
      ...
      [hal]: {
        ...prev[hal],
        lights: prev[hal].lights.map((intensity, index) =>
          index === laneId - 1 ? newValue : intensity
        ),
      },
    }));
  };
          ...
          {["Hal 1", "Hal 2", "Hal 3"].map((hal, index) => (
            <Card
              sx={{
                ...
                height: 150,
                margin: 6,
                backgroundColor: selectedHal === hal ? "lightblue" : "white",
                transition: "0.3s",
                "&:hover": {
                  backgroundColor: "lightblue",
                  boxShadow: "0 8px 16px 0 rgba(0,0,0,0.2)",
        ...
        {selectedHal && (
          <Grid container spacing={2} justifyContent="flex-end">
            <Grid item xs={4} sx={{ overflowY: "auto", maxHeight: 500 }}>
              <Box>
                {tempControlStates[selectedHal].lights.map(
                  (intensity, index) => (
                    <LightControlComp
                      key={index}
                      laneId={index + 1}
            ...
            <Grid item xs={4}>
              <TempHumiComp
                ...
                onTemperatureChange={(newValue) =>
                  updateTemperature(selectedHal, newValue)
                }
                onHumidityChange={(newValue) =>
                  updateHumidity(selectedHal, newValue)
                }
              />
            </Grid>
            <Grid item xs={4}>
